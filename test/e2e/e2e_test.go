	...
	Context("Manager", func() {
		...
		It("should ensure the metrics endpoint is serving metrics", func() {
			...
			cmd := exec.Command("sh", "-c", fmt.Sprintf(
				"kubectl create clusterrolebinding %s --clusterrole=vault-k8s-config-operator-metrics-reader --serviceaccount=%s:%s --dry-run=client -o yaml | kubectl apply -f -",
			...
			verifyMetricsAvailable := func(g Gomega) {
				...
				g.Expect(metricsOutput).To(ContainSubstring("HTTP_CODE:200"))
			}
...
func helmOverrideArgs(imageRef string) (string, error) {
	withoutTag, tag, err := splitImageTag(imageRef)
	if err != nil {
		return "", err
	}
...
func splitImageTag(imageRef string) (string, string, error) {
	lastSlash := strings.LastIndex(imageRef, "/")
	lastColon := strings.LastIndex(imageRef, ":")

	if lastColon > lastSlash {
		withoutTag := imageRef[:lastColon]
		tag := imageRef[lastColon+1:]
		if withoutTag == "" || tag == "" {
			return "", "", fmt.Errorf("invalid image reference %q", imageRef)
		}
		return withoutTag, tag, nil
	}
